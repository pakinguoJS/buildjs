...
*/
function setAlias(tpl, dest, jsalias, cssalias, version, jsreg, cssreg, vreg){
	if(arguments.length < 2){
		console.log('SetAlias function is required at least 2 paramters!');
		return;
	}
	
	version ? null : version	= 'new Date().getTime()';
	jsreg	? null : jsreg		= /('|")\{seajs_alias\}('|")/;
	cssreg	? null : cssreg 	= /('|")\{seacss_alias\}('|")/;
	vreg	? null : vreg		= /('|")\{version\}('|")/;
	...
	if( jsStat.mtime.getTime() > destStat.mtime.getTime() ||
		cssStat.mtime.getTime() > destStat.mtime.getTime() ||
	...
	function writeFile(){
		...
		data = data.replace(vreg, version);
