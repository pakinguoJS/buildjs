...
module.exports = {
	...
	compareExcludeRsync: compareExcludeRsync,
	compareOnly	 :	compareOnly
};


function compareExcludeRsync(src, dist, pattern, callback){
	if(arguments.length < 2){
		throw('Parameters Error!');
		return;
	}

	pattern ? null : pattern = 'view';

	var cmd = 'rsync -urtXog --delete --exclude=' + pattern + ' ' + src + ' ' + dist;

	var exec = require('child_process').exec;
	var start = new Date();
    exec(cmd, function(error, stdout, stderr) {
        if (error !== null) {
            console.log('Rsync exec error: ' + error);
            return;
        }

		var end = new Date();
		console.log("== COMPAREEXCLUDERSYNC TASK Total time: " + (end.getTime() - start.getTime()) + " ms");
		console.log("----------------------------------------");
			...
			if(FS.statSync(fpath).isDirectory()){
				...
				if(_setting.file.test(name)){
					if(_setting.unfold){
						...
						tpath = PATH.join(PATH.dirname(to), name);
						mv(fpath, tpath);
						__existList[tpath] = 1;
					}
				}
