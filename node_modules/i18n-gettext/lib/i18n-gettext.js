...
I18n.prototype.setFileType = function(type){
	...
	this._fileType = type || /(\.php|\.html|\.htm|\.htpl|\.tpl|\.js|\.css|\.txt)/g;
}
 ...
 */
I18n.prototype.xgettext = function(src, dist, merge, type, exist){
	if(!src){
	...
	if(!dist){
		if(FS.statSync(src).isDirectory()){
			...
			dist = PATH.dirname(src) + '/' + PATH.basename(src).replace(/\.[^\n\.]*$/, '');
		}
	}
	if(dist && !/\.[^\n]$/.test(dist)){
		if(!FS.existsSync(dist)){
	...
	switch(type){
		...
		case '.pot':
			if(FS.statSync(src).isDirectory()){
				merge ? (exist ? saveAsPoMergePro.call(this, src, exist) : saveAsPoMerge.call(this, src)) : traverseFiles.call(this, src, saveAsPoSingle);
			}else{
				saveAsPoSingle.call(this, src);
			}
			break;
		case '.xls':
		case '.xlsx':
	...
	function saveAsPoSingle(file){
		...
		// 保存
		check ? FS.writeFileSync(dist + '/' + filename + type, output, this._charset) : null;
	}
	...
	function saveAsPoMerge(dir){
		...
		FS.writeFileSync(dist + '/' + 'PO_MERGE_' + (new Date()).getTime() + type, output, this._charset);
	}
	 ...
	 * @param  {String}    dir  源文件夹
	 */
	function saveAsPoMergePro(dir, exist){
		// 新提取的
		var msgidList = extract.call(this, dir, true);
	...
	function extract(file, merge){
		...
		function logRepeat(){
			if(JSON.stringify(repeat).length > 2){
				...
				FS.writeFileSync(dist + '/' + 'PO_MERGE_REPEAT' + (new Date()).getTime() + '.log', log, this._charset);
			}
		}
	}
}
