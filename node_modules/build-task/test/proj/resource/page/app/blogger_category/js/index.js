...
define(function (require, exports, module) {
		...
		$.getJSON('get_more', {
			...
			var rows = $.map(data['data']['data']['list'], function (item, index) {
				var row = {
					...
					lastModified: item['last_update_h'],
					edit: [
						{
							type: 'link',
							clazz: 'modify-btn',
	...
	$('#release-btn').click(function (event) {
		$.getJSON('../release/article_category',
		function (data) {
			if (data['ret']) {
				IVYAlert(data['msg']);
				// $('#error-message').html('Sorry, your submission failed. Please try again.');
				...
				$('#live-preview')
					.foodiePreview().clear()
					.foodiePreview().addItem($('#modified-preview')
						.foodiePreview().getItemList()
					);
			}
		});
	});
